...
func emit(w io.Writer, ti TypeInfo, cfg Config) {
	...
	if cfg.Apply {
		emitApplyMethod(w, ti)
	}
}

func emitApplyMethod(w io.Writer, ti TypeInfo) {
	...
	fmt.Fprintln(w)
	if len(props) > 0 {
		fmt.Fprintf(w, "// Apply merges the delta into the target [%s]. For each property\n", ti.TypeName)
		fmt.Fprintln(w, "// carrying a Set instruction the entity field is overwritten; for Clear the")
		fmt.Fprintln(w, "// field is zeroed; for Skip the field is left untouched. The primary key")
		fmt.Fprintf(w, "// (%s) is always written.\n", strings.Join(keyLinks, ", "))
	} else {
		fmt.Fprintf(w, "// Apply merges the delta into the target [%s]. The primary key\n", ti.TypeName)
		fmt.Fprintf(w, "// (%s) is always written.\n", strings.Join(keyLinks, ", "))
	}
	fmt.Fprintln(w, "//")
	fmt.Fprintln(w, "// Generated with -apply.")
	fmt.Fprintf(w, "func (d *%s) Apply(target *%s) {}\n", deltaName, ti.TypeName)
}
